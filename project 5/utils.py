...
def create_indices(df:pd.DataFrame):
  ...
  df1['made_index'].iloc[0] = 1
  df1.loc[df1['interaction_term'] == df1['interaction_term'].max(), 'made_index'] = 200
  df1.loc[df1['interaction_term'] == df1['interaction_term'].median(), 'made_index'] = 50
  ...
  # Putting predictions into null made_index
  nulls['made_index'] = pipe.predict(nulls)

  # Imputation
  df1['made_index'].fillna(nulls['made_index'], inplace=True)
  df1.index = df.index

  # Replacing our max 200 and min 50 value with a linear interpolation
  df1.loc[df1['interaction_term'] == df1['interaction_term'].max(), 'made_index'] = np.nan
  df1.loc[df1['interaction_term'] == df1['interaction_term'].min(), 'made_index'] = np.nan
  df1['made_index'] = df1['made_index'].interpolate(method='linear')

  # Drop interaction term column
  df1.drop(columns=['interaction_term'], inplace=True)
